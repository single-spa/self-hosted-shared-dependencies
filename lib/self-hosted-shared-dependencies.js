...
/**
 * format package and version with split
 * @param {string} name package name
 * @param {string} version package version
 * @param {'@' || '/'} split  default '@'
 * @returns {string}
 */
function packageVersionGenerator(name, version, split) {
...
export async function build({
  ...
  const packages = !usePackageJSON
    ? packagesConfig
    : await packageConfigFromPackageJson();
    ...
    async function* processVersion(matchedVersion) {
      if (skipPackagesAtUrl) {
        const cacheCheckpath = new url.URL(
          `${packageVersionGenerator(p.name, matchedVersion)}/package.json`,
          skipPackagesAtUrl
        );
        const shouldSkip = await new Promise((resolve, reject) => {
          const stream = (
            cacheCheckpath.href.startsWith("http://") ? http : https
        ...
        if (shouldSkip) {
          return yield [
            logLevels.warn,
            `----> Skipping ${packageVersionGenerator(
              p.name,
              matchedVersion
            )} because it is already available at URL ${cacheCheckpath.href}`,
          ];
        }
      }
      ...
      const dir = path.resolve(
        outputDir,
        packageVersionGenerator(p.name, matchedVersion, customSplit)
      );

      yield [
        logLevels.warn,
        `----> Downloading and extracting ${packageVersionGenerator(
          p.name,
          matchedVersion
        )}`,
      ];
      ...
      await new Promise((resolve, reject) => {
        ...
        requestStream.on("timeout", () => {
          reject(
            Error(
              `Request timed out to download tarball for ${packageVersionGenerator(
                p.name,
                matchedVersion
              )}`
            )
...
async function packageConfigFromPackageJson() {
